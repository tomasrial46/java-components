	...
	private void initManager()
{
	...
	private void sendActuatorCommandtoCda(ResourceNameEnum resource, ActuatorData data) {
		// This is how an ActuatorData command gets passed to the CDA (via listener)
		if (this.actuatorDataListener != null) {
			this.actuatorDataListener.onActuatorDataUpdate(data);
		}

		// When using MQTT to communicate between GDA and CDA
	...
	private OffsetDateTime getDateTimeFromData(BaseIotData data) {
		...
		try {
			...
			// TODO: This won't be accurate, but should be reasonably close,
			// as the CDA will most likely have recently sent the data to the GDA
